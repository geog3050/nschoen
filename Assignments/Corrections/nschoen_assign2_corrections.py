...
def fight(participant1, participant2, first2attack):
    rounds = 0
    health1 = float(participant1[2])
    health2 = float(participant2[2])
    while (health1 > 0) and (health2 > 0):
        if first2attack == 1:
            attack = attack_multiplier(participant1[1], participant2[1])
            attackDamage = attack*float(participant1[3])
            health2 = health2-attackDamage 
        elif first2attack == 2:
            attack = attack_multiplier(participant2[1], participant1[1])
            attackDamage = attack*float(participant2[3])
            health1 = health1-attackDamage
        rounds = rounds+1
        if first2attack == 1:
            first2attack = 2
        elif first2attack == 2:
...
def tournament(participants):
    wins = []
    for element in range(len(participants)):
        wins.append(0) 
    for x in participants:
        for y in participants:
            if x==y:
                ...
                homeGame=fight(x,y,1)
                if homeGame[0]==1:
                    wins[participants.index(x)] = wins[participants.index(x)]+1
                elif homeGame[0]==2:
                    wins[participants.index(y)] = wins[participants.index(y)]+1
                awayGame=fight(x,y,2)
                if awayGame[0]==1:
                    wins[participants.index(x)] = wins[participants.index(x)]+1
                elif awayGame[0]==2:
                    wins[participants.index(y)] = wins[participants.index(y)]+1
